<template>
  <div v-bind:class="{'qae-item-checkbox':true, selected:selected}"  @click="click">
    ...
    <div>
      <input type="checkbox" v-model="minSelectionEnabled"/><span>最少<input type="text" class="input" v-model.number="minSelectionVal" :disabled="!minSelectionEnabled">项</span>
    </div>
    <div>
      <input type="checkbox" v-model="maxSelectionEnabled"/><span>最多<input type="text" class="input" v-model.number="maxSelectionVal" :disabled="!maxSelectionEnabled">项</span>
    </div>
  </div>
</template>
...
export default {
  ...
  computed:{
    ...
    minSelectionEnabled:{
      get(){
        return this.$store.state.qa.container.getItem(this.key).minSelection.enabled
      },
      set(newVal){
        const index=this.$store.state.qa.container.indexByKey(this.key)
        this.$store.commit("qa/updateValue", {path: `$.items[${index}].minSelection.enabled`,
          newVal: newVal,
          key:this.key
        })
      }
    ...
    minSelectionVal:{
      get(){
        return this.$store.state.qa.container.getItem(this.key).minSelection.val
      },
      set(newVal){
        const index=this.$store.state.qa.container.indexByKey(this.key)
        this.$store.commit("qa/updateValue", {path: `$.items[${index}].minSelection.val`,
          newVal: newVal,
          key:this.key
        })
      }
    ...
    maxSelectionEnabled:{
      get(){
        return this.$store.state.qa.container.getItem(this.key).maxSelection.enabled
      },
      set(newVal){
        const index=this.$store.state.qa.container.indexByKey(this.key)
        this.$store.commit("qa/updateValue", {path: `$.items[${index}].maxSelection.enabled`,
          newVal: newVal,
          key:this.key
        })
      }
    ...
    maxSelectionVal:{
      get(){
        return this.$store.state.qa.container.getItem(this.key).maxSelection.val
      },
      set(newVal){
        const index=this.$store.state.qa.container.indexByKey(this.key)
        this.$store.commit("qa/updateValue", {path: `$.items[${index}].maxSelection.val`,
          newVal: newVal,
          key:this.key
        })
      }
...
<style lang="less" scoped>
  ...
  .qae-item-checkbox>.title>.red{
    color:red
  }
  .qae-item-checkbox>.input{
    height:36px;
  ...
  .qae-item-checkbox .buttons-layout{
    display: inline-block;
  }
  .qae-item-checkbox .buttons{
    display: flex;
